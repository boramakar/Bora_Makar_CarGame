...
public class MainMenuScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        touchBtn.onClick.AddListener(() => { dataScript.CurrentControlType = EControlType.touch; inputUnderline.transform.position = touchBtn.transform.position; });
        keyboardBtn.onClick.AddListener(() => { dataScript.CurrentControlType = EControlType.keyboard; inputUnderline.transform.position = keyboardBtn.transform.position; });
        easyBtn.onClick.AddListener(() => { dataScript.CurrentDifficulty = EDifficulty.easy; dataScript.SetDifficulty(); difficultyUnderline.transform.position = easyBtn.transform.position; });
        normalBtn.onClick.AddListener(() => { dataScript.CurrentDifficulty = EDifficulty.normal; dataScript.SetDifficulty(); difficultyUnderline.transform.position = normalBtn.transform.position; });
        hardBtn.onClick.AddListener(() => { dataScript.CurrentDifficulty = EDifficulty.hard; dataScript.SetDifficulty(); difficultyUnderline.transform.position = hardBtn.transform.position; });

        //Underline the active settings
        Vector3 inputUnderlinePosition, difficultyUnderlinePosition;
        //
        if (dataScript.CurrentControlType == EControlType.touch)
            inputUnderlinePosition = touchBtn.transform.position;
        else
            inputUnderlinePosition = keyboardBtn.transform.position;

        if (dataScript.CurrentDifficulty == EDifficulty.easy)
            difficultyUnderlinePosition = easyBtn.transform.position;
        else if (dataScript.CurrentDifficulty == EDifficulty.normal)
